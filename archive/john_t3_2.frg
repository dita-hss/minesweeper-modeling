...
pred ableToGatherSpace[pre: Board, row: Int, col: Int]{
    some r,c: Int|{
        inBounds[r,c]
...
pred dumbAlgo[pre: Board, row: Int, col: Int]{
    ...
    initial[pre] implies inBounds[row,col]
    ...
    ableToGatherSpace[pre,row,col] implies (pre.adjacentMines[row][col] = 0 and pre.cells[row][col] = Revealed and adjacentHiddenChecker[pre,row,col])
    not ableToGatherSpace[pre,row,col] implies inBounds[row,col]
}
...
pred kindaSmartAlgo[pre: Board, row: Int, col: Int] {
   ...
   not ableToGatherSpace[pre,row,col] implies (not definetlyAMineOneTile[pre,row,col]) 
   inBounds[row,col]
}
...
-- so far : traces that follows basic algorithim of dumbAlgo
...
//run { game_trace_DumbAlgo } for 7 Board, 1 Game, 5 Int for {optimizer next is linear}

run { game_trace_kindaSmartAlgo } for 7 Board, 1 Game, 5 Int for {optimizer next is linear}

//run { game_trace_relativelySmartestAlgo } for 7 Board, 1 Game, 5 Int for {optimizer next is linear}